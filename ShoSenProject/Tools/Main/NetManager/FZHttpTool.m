...
+ (void)get:(NSString *)url parameters:(NSDictionary *)parameters httpToolSuccess:(FZSuccess)httpToolSuccess failure:(FZFailure)failure
{
    ...
    [manager GET:url parameters:parameters progress:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
        if (httpToolSuccess) {
                [self successWithResponseObject:responseObject url:url httpToolSuccess:httpToolSuccess failure:failure];
        }
...
+ (void)post:(NSString *)url parameters:(id )parameters isShowHUD:(BOOL)showHUD httpToolSuccess:(FZSuccess)httpToolSuccess failure:(FZFailure)failure
{
    ...
    if([url isEqualToString:UserLoginWechatUrl] || [url isEqualToString:UserWechaUnlock])
    {
        dicData = parameters;
    }else{
 ...
 *  @param failure 失败回调
 */
+ (void)postBoyWithUrl:(NSString *)url body:(NSString *)body showLoading:(BOOL)show httpToolSuccess:(FZSuccess)httpToolSuccess failure:(FZFailure)failure
{
    if (show) {
        HttpBegin
    }
    ...
    [[manager dataTaskWithRequest:request completionHandler:^(NSURLResponse * _Nonnull response, id  _Nullable responseObject, NSError * _Nullable error) {
        ...
        if (!error) {
            HttpEnd
            if (httpToolSuccess) {
                [self successWithResponseObject:dictionary url:url httpToolSuccess:httpToolSuccess failure:failure];
            }
        } else {
            failure(error);
        }
...
+ (RACSignal *)postWithURL:(NSString *)urlString withParamater:(NSDictionary *)parameter
{
    ...
    [manager POST:urlString parameters:parameter progress:^(NSProgress * _Nonnull uploadProgress) {
        ...
        if ([responseObject[@"code"] intValue] == 100) {
            ...
            MYLog(@"发送验证码成功%@",responseObject);
            if (responseObject[@"data"][@"message"]) {
                [ProgressHUD showSuccess:responseObject[@"data"][@"message"]];
            }
        
        }else if([responseObject[@"code"] intValue] == 997)
        {
...
+ (void)successWithResponseObject:(id)responseObject url:(NSString *)url httpToolSuccess:(FZSuccess)httpToolSuccess failure:(FZFailure)failure
{
    if (![responseObject isKindOfClass:[NSNull class]]) {
        if ([responseObject isKindOfClass:[NSDictionary class]]) {
            ...
            {
                if ([url isEqualToString:UserBaseSignLogin] || [url isEqualToString:UserLoginWechatUrl]) {
                    httpToolSuccess(responseObject);
                }else{
                    [ProgressHUD showError:responseObject[@"msg"]];
                }
