...
#define Spacing 10 // 每个item的间距  !! 这个宏已经没用了, 请用HXPhotoView 的 spacing 这个属性来控制

#define LineNum 4 // 每行个数  !! 这个宏已经没用了, 请用HXPhotoView 的 lineCount 这个属性来控制
...
//        }else {
            _addModel.thumbPhoto = [HXPhotoTools hx_imageNamed:@"circle_addImage"];
//        }
    }
    return _addModel;
}
...
- (void)setup {
    ...
    self.frame = CGRectMake(15, 0, kwidth - 30, kheight);
    ...
    UINib *headerNib = [UINib nibWithNibName:@"SSCirclePublishHeader" bundle: [NSBundle mainBundle]];
    [self.collectionView registerNib:headerNib forSupplementaryViewOfKind:UICollectionElementKindSectionHeader withReuseIdentifier:@"SSCirclePublishHeader"];
    UINib *footerNib = [UINib nibWithNibName:@"SSCirclePublishFooter" bundle: [NSBundle mainBundle]];
    [self.collectionView registerNib:footerNib forSupplementaryViewOfKind:UICollectionElementKindSectionFooter withReuseIdentifier:@"SSCirclePublishFooter"];
...
- (UICollectionReusableView *)collectionView:(UICollectionView *)collectionView viewForSupplementaryElementOfKind:(NSString *)kind atIndexPath:(NSIndexPath *)indexPath
{
    if (kind == UICollectionElementKindSectionHeader) {
        SSCirclePublishHeader *headerView = [collectionView dequeueReusableSupplementaryViewOfKind:UICollectionElementKindSectionHeader withReuseIdentifier:@"SSCirclePublishHeader" forIndexPath:indexPath];
        headerView.block = ^(NSString *contentStr) {
    ...
    SSCirclePublishFooter *footerView = [collectionView dequeueReusableSupplementaryViewOfKind:UICollectionElementKindSectionFooter withReuseIdentifier:@"SSCirclePublishFooter" forIndexPath:indexPath];
    [footerView.location_button addTarget:self action:@selector(feltLocation) forControlEvents:UIControlEventTouchUpInside];
...
- (void)feltLocation
{
    if (self.footerview.location_label.text.length > 0) {
        self.footerview.location_label.text = @"";
        [self.footerview.location_button setTitle:@"显示位置" forState:UIControlStateNormal];
        
    }else{
        [[SSLocationTool share]initBaiduLocationCallBack:^(NSString *obj) {
            self.footerview.location_label.text = obj;
            [self.footerview.location_button setTitle:@"不显示位置" forState:UIControlStateNormal];
            self.block(obj);
        }];
    }
}
...
-(CGSize)collectionView:(UICollectionView *)collectionView layout:(UICollectionViewLayout *)collectionViewLayout referenceSizeForHeaderInSection:(NSInteger)section{
    CGSize size={kwidth,95};
    return size;
}
//返回头footerView的大小
- (CGSize)collectionView:(UICollectionView *)collectionView layout:(UICollectionViewLayout*)collectionViewLayout referenceSizeForFooterInSection:(NSInteger)section
{
    CGSize size={kwidth,90};
    return size;
}
