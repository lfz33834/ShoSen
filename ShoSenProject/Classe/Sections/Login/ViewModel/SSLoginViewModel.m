...
@implementation SSLoginViewModel
...
- (void)initViewModel
{
    ...
    self.codeCommand = [[RACCommand alloc]initWithSignalBlock:^RACSignal *(id input) {
        ...
        btn.enabled = NO;
        self.time = 60;
        [btn setTitle:[NSString stringWithFormat:@"剩余%ld秒再次获取",(long)self.time] forState:UIControlStateNormal];
    ...
    self.loginCommand = [[RACCommand alloc]initWithSignalBlock:^RACSignal *(id input) {
        return [FZHttpTool postWithURL:LoginUrl withParamater:@{@"phone":self.phoneNum,@"smsCode":self.codeNum,@"invitorUserPhone":@""}];
    }];
}
