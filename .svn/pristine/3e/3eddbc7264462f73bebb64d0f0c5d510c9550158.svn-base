...
- (void)initPickerView{
    ...
    //确认按钮
    CGFloat titleHeight =  IT_KEYBOARD_HEIGHT - 150;
    UIView *titleView = [[UIView alloc] initWithFrame:CGRectMake(0, 10,kScreenWidth,titleHeight)];
    //titleView.backgroundColor = [UIColor blueColor];
    [self.addPickVview addSubview:titleView];
    
    UIButton *sureBtn = [UIButton buttonWithType:UIButtonTypeCustom];
    sureBtn.titleLabel.font = [UIFont systemFontOfSize:16];
    sureBtn.frame = CGRectMake(kScreenWidth - 80, 0, 60, titleHeight);
    [sureBtn setTitle:@"确定" forState:UIControlStateNormal];
    [sureBtn setTitleColor:kColor(@"666666") forState:UIControlStateNormal];
    ...
    pickerView.frame = CGRectMake(0, 20, kScreenWidth, 180);
...
- (UIView *)pickerView:(UIPickerView *)pickerView viewForRow:(NSInteger)row forComponent:(NSInteger)component reusingView:(UIView *)view{
    ...
    for(UIView *singleLine in pickerView.subviews)
    {
        ...
        {
            singleLine.backgroundColor = kColor(@"f8f8f8");
        }
    }
...
-(void)showChinaPickerView
{
    ...
    [UIView animateWithDuration:0.25f animations:^{
        float pikceY =  kScreenHeight - IT_KEYBOARD_HEIGHT - 64;
